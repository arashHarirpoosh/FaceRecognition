...
def do_recognize_person(all_face_img_addr):
    try:
        all_verified_identity = []
        all_preds = DeepFace.find(all_face_img_addr,
        ...
        for df in all_preds:
            ...
            if not df.empty:
                ...
                all_verified_identity.append(verified_identity)

        return all_verified_identity
...
def face_recognition(addr, n=5):
    ...
    for s in selected_img:
        ...
        all_faces.append(face_img_addr)

    predictions = do_recognize_person(all_faces)
